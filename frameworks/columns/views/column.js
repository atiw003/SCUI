...
SCUI.ColumnView = SC.View.extend(
/** @scope SCUI.ColumnView.prototype */ {
  
  classNames: ['scui-column-view'],
  ...
  columnListDelegate: SCUI.ColumnDelegate,
  ...
  _createViewForMyRole: function(context, controller) {
    ...
    var del                   = context.get('columnListDelegate');
  ...
  _setupMyRole: function(controller) {
    ...
    if (SC.kindOf(controller, SC.ArrayController)) {
      role = SCUI.COLUMN_VIEW_ROLE_LIST;
    }
    else {
      role = SCUI.COLUMN_VIEW_ROLE_META;
    }
